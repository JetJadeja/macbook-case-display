...
export function ShopItem({
  ...
  ownedByTeam = false,
  purchasedBy,
  purchasedByYou = false,
}: ShopItemProps) {
  const isOwned = isPurchased || ownedByTeam;

  return (
    <button
      onClick={canAfford && !isOwned ? onPurchase : undefined}
      ...
      <div className="relative h-full flex flex-col items-center justify-between p-4">
        ...
        {isOwned && (
          <div className="absolute top-2 right-2 bg-green-600 text-white text-xs font-bold px-2 py-1 rounded">
            {ownedByTeam
              ? (purchasedByYou ? "✓ BY YOU" : `✓ ${purchasedBy?.toUpperCase() || 'TEAMMATE'}`)
              : "✓ OWNED"}
          </div>
        )}

        {/* Icon */}
        <div className={`text-5xl mb-2 ${isOwned ? "opacity-50 line-through" : ""}`}>
          {icon}
        </div>

        {/* Name */}
        <div className={`text-sm font-bold text-white tracking-wider text-center ${isOwned ? "line-through opacity-70" : ""}`}>
          {name}
        </div>

        {/* Description */}
        <div className={`text-xs text-gray-300 text-center flex-1 flex items-center px-2 ${isOwned ? "opacity-50" : ""}`}>
          {description}
        </div>
        ...
        <div
          ...
          ${isOwned ? "bg-green-600/60" : canAfford ? "bg-yellow-600/80" : "bg-gray-600/80"}
        `}
        >
          <div className="text-xs text-white/70">{isOwned ? "PAID" : "COST"}</div>
